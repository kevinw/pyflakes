...
def flatten(nodes):
    if isinstance(nodes, ast.Name):
        ...
        return flatten(nodes.elts)

    flattened_nodes = []
    for node in nodes:
        if isinstance(node, ast.Tuple):
            flattened_nodes += flatten(node.elts)
        else:
            assert isinstance(node, ast.Name), repr(node)
...
class Checker(ast.NodeVisitor):
    ...
    def visit_For(self, node):
        ...
        '''
        for var in flatten(node.target):
                    ...
                    if upval is not None:
                        if upval.used and upval.used[0] is scope and name not in scope.globals:
                            ...
                            self.report(messages.UndefinedLocal,
                                        upval.used[1], name, upval.source.lineno, upval.source.col_offset)
